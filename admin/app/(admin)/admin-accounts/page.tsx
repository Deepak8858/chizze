...
export default function AdminAccountsPage() {
  ...
  const columns: ColumnDef<Admin, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ getValue }) => (
        <span
          className={`text-xs px-2 py-0.5 rounded capitalize font-medium ${ROLE_COLORS[(getValue() as string) ?? ""] ?? "text-text-muted bg-white/5"}`}
        >
          {((getValue() as string | undefined) ?? "read_only").replace(
            "_",
            " ",
          )}
        </span>
      ...
      cell: ({ row }) =>
        (row.original as any).$id !== (me as any)?.$id ? (
          <button
            ...
            className="text-status-error/50 hover:text-status-error transition-colors"
          >
            <Trash2 size={14} />
          </button>
        ) : (
