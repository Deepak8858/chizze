...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 py-1.5 border-b border-white/4 last:border-0">
...
export default function OrderDetailPage() {
  ...
  const customerName = order.customer_name?.trim() || "Not available";
  const customerPhone = order.customer_phone?.trim() || "";
  const customerAddress =
    [order.delivery_address_line?.trim(), order.delivery_city?.trim()]
      .filter(Boolean)
      .join(", ") || "Not available";
  const deliveryLat =
  ...
  const coordinateText = hasCoordinates
    ...
    : "Not available";
  const customerIdSuffix = order.customer_id
    ? order.customer_id.slice(-12)
    : "Not available";
        ...
        <div className="card p-4">
          ...
          <div className="flex items-start gap-0 overflow-x-auto pb-1">
            {TIMELINE.map((step, i) => {
              ...
              const timestamp = step.field
                ? (order[step.field] as string | undefined)
                : null;
              return (
                <div key={step.status} className="flex items-center shrink-0">
                  <div className="flex flex-col items-center gap-1.5 min-w-20 text-center">
                    ...
                    >
                      {done ? (
                        <CheckCircle
                          size={14}
                          className={
                            active ? "text-white" : "text-status-success"
                          }
                        />
                      ) : (
                    ...
                    <p
                      className={`text-[10px] font-medium leading-tight ${active ? "text-brand-400" : done ? "text-white" : "text-text-muted"}`}
                    >
                      {step.label}
                    </p>
                    {timestamp && (
                      <p className="text-[9px] text-text-muted">
                        {formatDateTime(timestamp).split(",")[1]?.trim()}
                      </p>
                    )}
                  </div>
                  {i < TIMELINE.length - 1 && (
                    <div
                      className={`h-0.5 w-8 shrink-0 -mt-4 ${
                        currentIdx > STATUS_ORDER.indexOf(step.status)
        ...
        <div className="card p-4 border-status-error/30 bg-status-error/5">
          ...
          {order.cancelled_at && (
            <p className="text-xs text-text-muted">
              At: {formatDateTime(order.cancelled_at)}
            </p>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
                ...
                items.map(
                  (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-3 h-3 rounded-sm border shrink-0 ${item.is_veg ? "border-status-success" : "border-status-error"}`}
                        >
                          <span
                            className={`block w-1.5 h-1.5 rounded-full m-auto mt-0.5 ${item.is_veg ? "bg-status-success" : "bg-status-error"}`}
                          />
                        </span>
          ...
          <InfoCard title="Pricing Breakdown" icon={<IndianRupee size={12} />}>
            ...
            <Row
              label="Platform Fee"
              value={formatCurrency(order.platform_fee)}
            />
            <Row label="GST" value={formatCurrency(order.gst)} />
            {order.tip > 0 && (
              <Row label="Tip" value={formatCurrency(order.tip)} />
            )}
            {order.discount > 0 && (
              <Row
        ...
        <div className="space-y-4">
          ...
          <InfoCard title="Customer" icon={<User size={12} />}>
            ...
            {order.delivery_instructions && (
              <div className="mt-2 p-2 rounded bg-white/3 border border-white/4">
            ...
            <Row
              ...
              value={
                <span className="font-mono text-brand-400 text-[10px]">
                  {order.restaurant_id.slice(-12)}
                </span>
              }
            />
            <Row
              label="Est. Time"
              value={`${order.estimated_delivery_min} min`}
            />
          </InfoCard>
          ...
          {order.delivery_partner_id ? (
            <InfoCard title="Delivery Rider" icon={<Bike size={12} />}>
              {order.delivery_partner_name && (
                <Row label="Name" value={order.delivery_partner_name} />
              )}
              ...
              <Row
                ...
                value={
                  <span className="font-mono text-[10px]">
                    {order.delivery_partner_id.slice(-12)}
                  </span>
                }
              />
            </InfoCard>
          ...
          <InfoCard title="Order Meta" icon={<Clock size={12} />}>
            <Row
              label="Order ID"
              value={<span className="font-mono text-[10px]">{order.$id}</span>}
            />
