...
export default function NotificationsPage() {
  ...
  const sendMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      toast.error(serverMessage || "Failed to send notification");
    },
  });

  const sendNotification = () => {
    if (form.target_type === "specific_user" && !form.target_id.trim()) {
      toast.error("User ID is required for specific user notifications");
      return;
    }
    sendMutation.mutate({ ...form, target_id: form.target_id.trim() });
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-muted">
          Broadcast notifications directly to app users in real time
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-5 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Send size={14} className="text-brand-400" /> Send Notification
          </h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-surface-200 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-500"
              >
                {["promo", "order_update", "system"].map((t) => (
                  <option key={t} value={t}>
                    {t}
                  </option>
                ))}
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {isLoading
              ...
              : data?.data?.map((n) => (
                  <div
                    key={n.$id}
                    className="flex items-start gap-3 p-3 rounded-lg bg-surface-200"
                  >
